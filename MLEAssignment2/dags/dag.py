...
with DAG(
    ...
    bash_command='cd /opt/airflow/scripts && python3 feature_processing.py --snapshotdate "{{ ds }}" --task bronze_clickstream'
    )

    bronze_attributes_task = BashOperator(
        task_id='bronze_attributes', 
        bash_command='cd /opt/airflow/scripts && python3 feature_processing.py --snapshotdate "{{ ds }}" --task bronze_attributes'
    )

    bronze_financials_task = BashOperator(
        task_id='bronze_financials',
        bash_command='cd /opt/airflow/scripts && python3 feature_processing.py --snapshotdate "{{ ds }}" --task bronze_financials'
    )

    silver_clickstream_task = BashOperator(
    task_id='silver_clickstream',
    bash_command='cd /opt/airflow/scripts && python3 feature_processing.py --snapshotdate "{{ ds }}" --task silver_clickstream'
    )

    silver_attributes_task = BashOperator(
        task_id='silver_attributes', 
        bash_command='cd /opt/airflow/scripts && python3 feature_processing.py --snapshotdate "{{ ds }}" --task silver_attributes'
    )

    silver_financials_task = BashOperator(
        task_id='silver_financials',
        bash_command='cd /opt/airflow/scripts && python3 feature_processing.py --snapshotdate "{{ ds }}" --task silver_financials'
    )

    # gold_feature_store = DummyOperator(task_id="gold_feature_store")
    gold_feature_store = BashOperator(
        task_id='gold_features',
        bash_command='cd /opt/airflow/scripts && python3 feature_processing.py --snapshotdate "{{ ds }}" --task gold_features --type training'
    )
    ...
    model_inference = BashOperator(
        ...
        bash_command = 'cd /opt/airflow/scripts && python3 model_inference.py --snapshotdate "{{ ds }}"'
    )

    online_features = BashOperator(
        task_id='gold_features',
        bash_command='cd /opt/airflow/scripts && python3 feature_processing.py --snapshotdate "{{ ds }}" --task gold_features --type inference', 
