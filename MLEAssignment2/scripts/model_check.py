...
# def promote_best_model(experiment_name="loan-default-predictor", metric="test_f1"): 
#     """
...
#     # Get runs sorted by performance metric
#     runs = client.search_runs(
#         experiment_ids=[experiment.experiment_id], 
#         order_by=[f"metrics.{metric} DESC"],
#         filter_string="tags.model_type != ''"  # Only get runs with model_type tag
#     )
...
#     # Use the artifact path from your script
#     model_uri = f"runs:/{run_id}/loan-default-predictor"
    
#     print(f"Promoting best model:")
#     print(f"  Run ID: {run_id}")
#     print(f"  Model Type: {model_type}")
#     print(f"  {metric}: {best_run.data.metrics.get(metric, 'N/A')}")
#     print(f"  OOT F1: {best_run.data.metrics.get('oot_f1', 'N/A')}")

#     # Register model
#     result = mlflow.register_model(model_uri, 'loan-default-model')

#     # Set alias "champion"
#     client.set_registered_model_alias('loan-default-model', "champion", version=result.version)
    
#     print(f"Model registered as version {result.version} with 'champion' alias")
#     return model_type